...
while running:
    ...
    # Collision character coordinate update
    character_rect = character.get_rect()
    character_rect.left = character_x_pos
    character_rect.top = character_y_pos

    # Collision Ball Coordinate
    for ball_idx, ball_val in enumerate(balls):     # enumerate() = list 의 index, value 를 반환
        ball_pos_x = ball_val["pos_x"]
        ball_pos_y = ball_val["pos_y"]
        ball_img_idx = ball_val["img_idx"]
        ...
        # Collision character and balls
        if character_rect.colliderect(ball_rect):
            running = False
            break

        # Collision weapon and balls
        for weapon_idx, weapon_val in enumerate(weapons):
            ...
            # Collision Check weapon and ball
            if weapon_rect.colliderect(ball_rect):
                weapon_to_remove = weapon_idx
                ball_to_remove = ball_idx

                # if NOT Smallest Ball, divide smaller ball
                if ball_img_idx < 3:
                    # ball size at collision time
                    ball_width = ball_rect.size[0]
                    ball_height = ball_rect.size[1]
                    # ball size after divided
                    small_ball_rect = ball_images[ball_img_idx + 1].get_rect()
                    small_ball_width = small_ball_rect[0]
                    small_ball_height = small_ball_rect[1]

                    # Smaller ball to left
                    balls.append({
                        "pos_x": ball_pos_x + (ball_width / 2) - (small_ball_width / 2),  # 공의 x 좌표
                        "pos_y": ball_pos_y + (ball_height / 2) - (small_ball_height / 2),  # 공의 y 좌표
                        "img_idx": ball_img_idx + 1,  # 공의 이미지 인덱스(ball_images)
                        "to_x": -3,  # x축 이동방향
                        "to_y": -3,  # y축 이동방향
                        "init_speed_y": ball_speed[ball_img_idx + 1]  # 최초 속도 y = ball_speed 의 0번째 값으로 적용
                    })
                    # Smaller ball to right
                    balls.append({
                        "pos_x": ball_pos_x + (ball_width / 2) - (small_ball_width / 2),  # 공의 x 좌표
                        "pos_y": ball_pos_y + (ball_height / 2) - (small_ball_height / 2),  # 공의 y 좌표
                        "img_idx": ball_img_idx + 1,  # 공의 이미지 인덱스(ball_images)
    ...
    # Remove collision ball and weapon
    if ball_to_remove > -1:
...
# msg for game over
msg = game_font.render(game_result, True, (255, 200, 0))
